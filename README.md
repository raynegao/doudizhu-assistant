...
助手可以先于斗地主启动。未检测到游戏窗口时会持续等待；窗口打开后自动识别。游戏窗口最小化时，小窗会立即显示无法识别并暂停推荐，恢复窗口后重新探测。若最小化发生在已经开始跟踪的牌局中，为避免漏事件污染状态，该局会等待下一局完整初始化。
...
跟踪开始后，自己的出牌以连续两帧稳定的“上一手牌 − 当前手牌”生成事件，不再依赖会与游戏按钮重叠的自己出牌 ROI。对手场上牌会同时使用牌点图形和余牌数字变化确认；若当前 trick 中两名对手的余牌均未减少，而界面已经重新出现自己的回合按钮，状态机会依次生成两次“不出”、清空当前待压牌并立即计算新的自由出牌 Top-3。
