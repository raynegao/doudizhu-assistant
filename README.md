...
助手在未建模或窗口中断后，只要连续 2 帧稳定看到自己的回合、角色、手牌、左右余牌和当前待压牌，就会自动扫描当前牌局并恢复推荐。置顶窗同时提供“扫描当前牌局”按钮用于随时强制重扫；一次点击会自动重试最多 8 个可用帧，以吸收 macOS OCR 的偶发漏读，不必反复点击。手牌优先使用本机真实牌点模板修正 CNN，场上紧密重叠的对子也会先逐张分割再识别。中途画面无法还原此前牌点时，系统只记录未知历史牌张数，并从剩余未知牌中同时采样对手手牌和历史弃牌池，界面会明确显示近似模型警告。地主、余牌或当前牌在重试后仍无法稳定确认时，本次扫描失败并保持推荐暂停。
