...
助手可以先于斗地主启动。未检测到游戏窗口时会持续等待；窗口打开后自动识别。游戏窗口最小化时，小窗会立即显示无法识别并暂停推荐，恢复窗口后重新探测。活动牌局会把本局初始手牌参考和最后一个可信 revision 保存到本地忽略目录；助手在 30 分钟内意外重启时可恢复到该可信状态。若窗口不可见期间可能漏掉场上动作，仍会保持暂停并等待下一局完整初始化，不能用断点恢复伪造未知事件。
...
窗口级截图直接用目标窗口图像推导 Retina 比例，不再额外读取整个桌面，因此从 Terminal 桌面快捷方式启动时不会因全屏 `ImageGrab` 中断。macOS 仍会按启动应用单独检查“屏幕与系统音频录制”权限；首次从 Terminal 双击启动需允许 Terminal，授权后重新打开快捷方式。后台识别线程若异常，小窗会直接显示错误原因，不会一直停在“正在启动”。

在完整开局中，自己可见的 20 张牌是“自己为地主”的强结构信号，优先级高于容易受大面积固定背景干扰的整块角色模板；系统据此把另外两家确定为农民，再按 20 张切分和识别手牌。
