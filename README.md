...
助手可以先于斗地主启动。未检测到游戏窗口时会持续等待；窗口打开后自动识别。游戏窗口最小化时，小窗会立即显示无法识别并暂停推荐，恢复窗口后重新探测。活动牌局会把本局初始手牌参考和最后一个可信 revision 保存到本地忽略目录；同一次助手运行中的识别子进程异常退出会自动拉起，并用该运行会话的断点继续。重新双击启动助手会生成新的会话，不会加载上一次已停止进程的旧牌局，以免把停机期间漏掉的出牌伪造成连续状态。若窗口不可见期间可能漏掉场上动作，仍会保持暂停并等待下一局完整初始化。
...
助手在未建模或窗口中断后，只要连续 2 帧稳定看到自己的回合、角色、手牌、左右余牌和当前待压牌，就会自动扫描当前牌局并恢复推荐。自己的回合优先按提示/出牌按钮的大面积黄色结构判断，避免只有 active 模板时被固定蓝色背景误导。左右余牌使用黄色数字分割和归一化字形轮廓匹配，不再依赖系统把单个描边数字当作文本；置顶窗同时提供“扫描当前牌局”按钮用于随时强制重扫，一次点击会自动重试最多 8 个可用帧，不必反复点击。手牌优先使用本机真实牌点模板修正 CNN，场上紧密重叠的对子也会先逐张分割再识别。中途画面无法还原此前牌点时，系统只记录未知历史牌张数，并从剩余未知牌中同时采样对手手牌和历史弃牌池，界面会明确显示近似模型警告。地主、余牌或当前牌在重试后仍无法稳定确认时，本次扫描失败并保持推荐暂停。
...
跟踪开始后，自己的出牌以连续两帧稳定的“上一手牌 − 当前手牌”生成事件，不再依赖会与游戏按钮重叠的自己出牌 ROI。手牌切分会始终锚定牌顶，即使“您当前没有牌大过对方”等横幅遮住牌身，也不会把牌底白块当作牌点；同一局的初始牌点参考会辅助后续稳定识别。对手场上牌会同时使用牌点图形和余牌数字变化确认；若短暂的“不出”文字恰好落在两个采样帧之间，状态机会用后续行动者已验证的余牌减少和出牌反推遗漏的过牌事件。若当前 trick 中两名对手的余牌均未减少，而界面已经重新出现自己的回合按钮，则依次生成两次“不出”、清空当前待压牌并立即计算新的自由出牌 Top-3。
