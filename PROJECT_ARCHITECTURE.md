...
- 未建模或窗口中断后，跟踪器会在自己回合的场面连续稳定 2 帧时自动重建；Tk UI 的“扫描当前牌局”通过进程间命令队列提供强制重扫，并在最多 8 个可用帧内吸收 OCR 偶发漏读。手牌用真实牌点指纹修正 CNN，紧密重叠场面牌按垂直边缘逐张切分。中盘缺失的历史牌点只计入 `hidden_played_count`，决策采样时把这部分作为未知历史弃牌池，不伪造具体牌点。
