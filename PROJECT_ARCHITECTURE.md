...
- Pipeline 会在本地忽略目录保存当前局初始手牌参考和最后一个可信状态；同一 UI 运行会话中的识别子进程短时重启可恢复 revision，新启动的 UI 会话拒绝旧断点，完整新局稳定出现后会自动替换旧状态。
- 角色徽标使用局部字色信号（金色地主、白色农民）跨座位识别，20 张本地手牌作为地主结构交叉校验；稳定地主位置变化会被视为新局边界并立即阻断推荐。
- Tk UI 与截图/Vision 识别分进程运行，隔离 macOS CoreFoundation 辅助进程；识别进程异常退出可在原小窗内自动拉起。
- 未建模或窗口中断后，跟踪器会在自己回合的场面连续稳定 2 帧时自动重建；自己回合优先按黄色出牌控件的结构信号判断，整块 turn 模板只作回退。Tk UI 的“扫描当前牌局”通过进程间命令队列提供强制重扫，并在最多 8 个可用帧内吸收 OCR 偶发漏读。手牌用随代码分发的匿名二值牌点特征和本地真实模板修正 CNN，紧密重叠场面牌按垂直边缘逐张切分。中盘缺失的历史牌点只计入 `hidden_played_count`，决策采样时把这部分作为未知历史弃牌池，不伪造具体牌点。
